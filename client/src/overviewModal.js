...
const OverviewModal = ({ visible, onClose, selectedRoomId, roomInfo }) => {
  ...
  const handleLeaveRoom = async () => {
    try {
      ...
      if (confirm) {
        ...
        const response = await axiosInstance.delete(`/common-rooms/${selectedRoomId}/leaveRoom`);
      ...
      if (error.response && error.response.status === 400 && error.response.data.message === "Admins cannot leave the room") {
        notification.info({
          message: "Admin Restriction",
          description: "As an admin, you cannot leave this room. You can assign another admin or delete the room.",
        });
  ...
  return (
    ...
    >
      ...
      {/* Settings/Options */}
      <Card style={{ marginTop: "10px" }}>
        <Button type="default" block style={{ marginTop: "5px" }}>
          Leave Room
        </Button>
        <Button type="danger" block style={{ marginTop: "5px" }} onClick={handleDeleteRoom}>
          Delete Room
        </Button>
      </Card>
    </Modal>
