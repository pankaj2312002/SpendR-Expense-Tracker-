...
const RoomContent = ({ selectedRoomId, setIsLinkAvailable, userId }) => {
  console.log(`selectedRoomId ki id : ${selectedRoomId}`);
  ...
  useEffect(() => {
    const fetchRoomDetails = async () => {
      try {
        ...
        if (response.data) {
          ...
          // localStorage me save , so that addMemberModal shoe karwate time access kar paye
    ...
    if (selectedRoomId) {
      fetchRoomDetails();
      console.log(`fetched roomName is : ${roomInfo.roomName}`);
      console.log(`fetched list of members is : ${roomInfo.members}`);
    }
  ...
  useEffect(() => {
    const fetchRoomTransactions = async () => {
      try {
        ...
        if (response.data) {
          ...
          console.log(
            `these are transactions of members : ${memberTransactions}`
          ); // Assuming the response contains members;
    ...
    if (selectedRoomId) {
      ...
      console.log(`these are transactions of members : ${memberTransactions}`); // Assuming the response contains members;
    }
  }, [selectedRoomId]);
  ...
  const handleDeleteTransaction = async (memberId, transactionId) => {
    console.log (`delete req is at frontend function`)
    try {
      ...
      notification.error({ message: "Failed to delete transaction" });
    }
  };  
                  ...
                  renderItem={(transaction, i) => (
                    ...
                    >
                      ...
                      <Button
                        ...
                        onClick={() =>
                          handleDeleteTransaction(member.id, transaction.id)
                        }
                        style={{
                          position: "absolute",
                          top: "5px",
